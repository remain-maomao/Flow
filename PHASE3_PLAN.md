...
**问题**：当前 500×640 窄长比例不合理，像手机竖屏。

**方案**：改为 680×500（横版黄金比例），让信息卡片正常展示。

**修改文件**：`main.kt` 中 `rememberWindowState(width = ..., height = ...)`
...
**问题**：窗口不够高时，内容（模式条 + 计时 + 桌面窗口 + 浏览器 + 设置面板 + 倍速 + 按钮）被截断无法查看。

**方案**：将主 UI 区域包裹在 `verticalScroll` 中。

**修改文件**：`ui/App.kt`

**验证**：缩小窗口高度 → 可以滚轮上下滚动查看全部内容。
